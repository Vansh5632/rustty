...
impl IndexManager{
    pub fn new(storage:LsmStorage) ->Self{
        Self{
            indexes:HashMap::new(),
            storage,
        }
    ...
    pub async fn update_index(
        ...
        if let Some(descriptor) = self.indexes.get(index_name){
            ...
            self.storage.put(&index_key,&[]).await?;
        }
        Ok(())
    }
    ...
    pub async fn lookup_index(
        ...
        let records = self.storage.scan(&prefix).await?;

        let record_keys = records.into_iter().map(|(key,_)| self.extract_record_key(&key)).collect();
